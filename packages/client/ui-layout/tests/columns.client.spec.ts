...
describe('computeColumns', () => {
  it('step 1: everything fits at preferred widths', () => {
    ...
    expect(cols).toEqual({ sidebar: 220, center: 1920 - 220 - 360, details: 360 })
  })
  ...
  it('step 2: details shrinks first, center pinned at min', () => {
    // 220 + 360 + 640 = 1220 > 1180; details concedes to 1180-220-640 = 320.
    const cols = computeColumns(1180, open(SIDEBAR_DEFAULT), open(DETAILS_DEFAULT))
    expect(cols).toEqual({ sidebar: 220, center: CENTER_MIN, details: 320 })
  })
  ...
  it('step 3: details auto-closes when its min still starves center — sidebar holds its preference', () => {
    // 220 + 300 + 640 = 1160 > 1150 → details 0; sidebar untouched: center = 1150-220 = 930.
    const cols = computeColumns(1150, open(SIDEBAR_DEFAULT), open(DETAILS_DEFAULT))
    expect(cols).toEqual({ sidebar: 220, center: 930, details: 0 })
  })

  it('the sidebar never concedes: center absorbs the deficit below CENTER_MIN', () => {
    // 700 < 220+640: sidebar keeps 220, center takes 480 < CENTER_MIN.
    const cols = computeColumns(700, open(SIDEBAR_DEFAULT), closed(DETAILS_DEFAULT))
    expect(cols).toEqual({ sidebar: SIDEBAR_DEFAULT, center: 480, details: 0 })
  })
