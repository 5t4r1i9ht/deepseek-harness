...
describe('BrandWordmark', () => {
  it('can render the name artwork with or without its leading mark', () => {
    ...
    // The HARNESS badge plate is dropped on this branch: 54px narrower
    // than upstream in both variants.
    expect(svg.getAttribute('width')).toBe('128')
    expect(svg.getAttribute('viewBox')).toBe('0 0 128 24')

    view.rerender(<primitives.BrandWordmark includeMark={false} />)
    expect(svg.getAttribute('width')).toBe('102')
    expect(svg.getAttribute('viewBox')).toBe('26 0 102 24')
  })
})
